...
public:
  ...
  void set_ephemeral(char *ephemeral)
  {
    if (SSL_CTX_set1_groups_list(m_ctx, ephemeral) != 1){
      puts("cannot set supported group");
    }

  }
  void load_server_creds(char *cert_path, char *key_path)
  {
    int err;
    err = SSL_CTX_use_certificate_file(m_ctx, cert_path ,SSL_FILETYPE_PEM);
    //SSL_CTX_use_certificate_file(m_ctx, "../rustls/test-ca/rsa/end.cert",SSL_FILETYPE_PEM); "/home/anonmous/Documents/C_projects/TLS_server/Certs/secp256k1.crt"
    assert(err == 1);
    err = SSL_CTX_use_PrivateKey_file(m_ctx, key_path , SSL_FILETYPE_PEM);
    //err = SSL_CTX_use_PrivateKey_file(m_ctx,"../rustls/test-ca/rsa/end.key", SSL_FILETYPE_PEM); "/home/anonmous/Documents/C_projects/TLS_server/Certs/secp256k1.key"
...
void SGX_warmup(Context &server_ctx, Context &client_ctx)
{
    Conn server(server_ctx.open());
    Conn client(client_ctx.open());
...
SGX_warmup(server_ctx,client_ctx);
...
int ephemeral_info(char *ephemeral)
{
  printf("Ephemeral (ECDHE) is being set manually to %s \n",ephemeral);
  return 1;
}

int cert_path_info(char *cert_path, char *key_path)
{
  printf("server cert path  is being set manually to %s \n",cert_path);
  printf("server key path  is being set manually to %s \n \n",key_path);
  return 1;
}
...
char *get_ephemeral()
{
  ...
  const char ephemeral[10] = getenv("BENCH_EPHEMERAL");
  if (ephemeral) {
    return size_t(work * atof(multiplier));
  }
  return work;
...
int main(int argc, char **argv)
{
  ...
  if (!strcmp(argv[1], "handshake")) {
    ephemeral_info(argv[3]);
    client_ctx.set_ephemeral(argv[3]);
    cert_path_info(argv[4],argv[5]);
    server_ctx.load_server_creds(argv[4],argv[5]);
  }
