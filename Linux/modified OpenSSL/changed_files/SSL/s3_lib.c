...
int ssl_derive(SSL *s, EVP_PKEY *privkey, EVP_PKEY *pubkey, int gensecret)
{
    ...
    size_t pmslen = 0;
    EVP_PKEY_CTX *pctx;
    if ((s->LURK_Request->ephemeral->method != secret_generated) || (SSL_is_server(s) == 0)) //LURK
	{
    ...
    if (SSL_is_server(s) == 1  && s->LURK_Request->operation_mode != OPRATION_MODE_KEY_LESS) {
        ...
        {
            pmslen = (size_t)s->LURK_Request->ephemeral->shared_secret->shared_secret_len;
            pms = OPENSSL_malloc(pmslen);
            if (pms == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL_DERIVE,
                    ERR_R_MALLOC_FAILURE);
            ...
            memcpy(pms, s->LURK_Request->ephemeral->shared_secret->shared_secret, pmslen);
        }
		
		modify_shared_secret_SInitCertVerifyRequest(s->LURK_Request, pms, (int)s->s3->group_id);
		if (s->LURK_Request->operation_mode == OPRATION_MODE_1_EARLY_2_HS_SIG_AP ||
		...
		{
			s->LURK_Request->secret_request = get_sec_req_by_label((unsigned char *)"early");
			s->LURK_Request->md_index = ssl_get_algorithm2(s);
			if (LURK_OPENSSL(s->LURK_Request, s->LURK_Response, (unsigned char *)"early") == NULL)
            {
				goto err;
			}
    ...
    if (gensecret) {
        ...
        if (SSL_IS_TLS13(s)) {
            ...
            if (!s->hit)
            {
                ...
                //rv = 1;
                if (SSL_is_server(s) == 0 || s->LURK_Request->operation_mode == OPRATION_MODE_KEY_LESS) { //|| SSL_is_server(s) == 1
                ...
                //TESTING
                if (SSL_is_server(s) == 1 && s->LURK_Request->operation_mode != OPRATION_MODE_KEY_LESS) {
                    //compare_secrets(s->early_secret, s->LURK_Response->secret_list[9].secret_data,32);
                    rv = 1;
                }
            }
            else
            ...
            if (SSL_is_server(s) == 1 && s->LURK_Request->operation_mode != OPRATION_MODE_KEY_LESS) {
                rv = 1;
                //compare_secrets(s->handshake_secret, s->LURK_Response->secret_list[10].secret_data,32);
            }
        } else {
