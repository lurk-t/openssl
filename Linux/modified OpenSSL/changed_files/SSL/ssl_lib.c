...
SSL *SSL_new(SSL_CTX *ctx)
{
    ...
    /*
	* LRUK inits 
	* 1- new  and init the req and responce for LURK CS
	*/
	s->LURK_Request = new_LURK_request();
	s->LURK_Response = new_LURK_response();
	if (s->LURK_Request == NULL ||s->LURK_Response == NULL){
        goto err;
    }
