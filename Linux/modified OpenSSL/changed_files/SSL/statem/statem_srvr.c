...
int LURK_freshness_func(unsigned char *old_rand, struct SInitCertVerifyRequest *request)
{
	size_t new_rand_size = 0;
	int old_rand_size = 32;
	set_server_old_rand((void *)old_rand, request);
	...
	EVP_DigestFinal_ex(mdctx, old_rand, &new_rand_size);
...
static int tls_early_post_process_client_hello(SSL *s)
{
     ...
     */
    {
        ...
        //LURK
		LURK_freshness_func(s->s3->server_random, s->LURK_Request);
    }
...
int tls_construct_new_session_ticket(SSL *s, WPACKET *pkt)
{
    ...
    if (SSL_IS_TLS13(s)) {
        ...
        if (SSL_is_server(s) == 1){
            ...
            if (LURK_s_new_tickets(s->LURK_Response,tick_nonce,TICKET_NONCE_SIZE, s->LURK_Request,
            s->session->session_id,s->session->session_id_length,s->num_tickets, s->sent_tickets ,s->session->master_key, hashlen) != 1){
                goto err;
            }
        }
