                      ...
                      int external)
{
    ...
    if (SSL_is_server(s) == 1){
        ...
        modify_Handshake_Hash_SInitCertVerifyRequest(s->LURK_Request, hash, hashsize);
        if (LURK_get_stateful_ticket_binder_key(s->LURK_Request,s->LURK_Response,
            sess->master_key) != 1){
                goto err;
            }
        memcpy(binderkey,s->LURK_Response->secret_list[SECRET_LIST_INDEX_BINER_KEY].secret_data ,hashsize);
    }
