...
/*
 LURK
 This function only works for TLS 1.3 and it would give the hash of
  the handshakes that are needed to be signed for TLS 1.3 cert verify
*/
int get_LURK_cert_verify_tbs_data(SSL *s, unsigned char *tls13tbs,
	void **hdata, size_t *hdatalen)
{
	if (SSL_IS_TLS13(s)) {
		size_t hashlen;
		...
		if (s->statem.hand_state == TLS_ST_CR_CERT_VRFY || s->statem.hand_state == TLS_ST_SR_CERT_VRFY) {
			memcpy(tls13tbs, s->cert_verify_hash, s->cert_verify_hash_len);
			hashlen = s->cert_verify_hash_len;
		}
		else if (!ssl_handshake_hash(s, tls13tbs, EVP_MAX_MD_SIZE, &hashlen)) {
			/* SSLfatal() already called */
			return 0;
		}
...
int tls_construct_cert_verify(SSL *s, WPACKET *pkt)
{
    ...
    //LURK 
	if (s->LURK_Request->operation_mode == OPRATION_MODE_1_EARLY_HS_2_SIG_AP ||
		s->LURK_Request->operation_mode == OPRATION_MODE_1_EARLY_2_HS_3_SIG_AP_4_RESUMPTION ||
	...
	{
		unsigned char LURK_tls13tbs[EVP_MAX_MD_SIZE];
		void *LURK_hdata;
		size_t LURK_hdatalen = 0;
        s->LURK_Request->md_index = ssl_get_algorithm2(s);
		if (get_LURK_cert_verify_tbs_data(s, LURK_tls13tbs, &LURK_hdata, &LURK_hdatalen) <= 0) {
			goto err;
		}
		modify_Handshake_Hash_Signature_SInitCertVerifyRequest(s->LURK_Request, (unsigned char *)LURK_hdata, LURK_hdatalen, lu->sigalg);
        // to fingure print the right private key
        finger_print_signature(s->LURK_Request, EVP_PKEY_size(pkey));
		s->LURK_Request->secret_request = 0;
		if (LURK_OPENSSL(s->LURK_Request, s->LURK_Response, NULL) == NULL) {
			goto err;
		}
	}
    ...
    */
    ...
    //compare_secrets(sig, s->LURK_Response->signature->signature, s->LURK_Response->signature->signature_size);
	memcpy(sig, s->LURK_Response->signature->signature, s->LURK_Response->signature->signature_size);
	siglen = s->LURK_Response->signature->signature_size;
