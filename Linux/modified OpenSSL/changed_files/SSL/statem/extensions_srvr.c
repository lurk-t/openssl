                                        ...
                                        size_t chainidx)
{
    ...
    //LURK
	/*
	* if the LURK CS is expected to generate the secrets and public key
	* it would generate they private key but keep it secert and only return
	* public key (since it is needed in the server hello) and share secret
	* (since its needed for generating the hs/app secerets)
	* public key: responce->ephemeral->key_ex
	* share secret: request->ephemeral->share_secret
	*/
	size_t len;
	unsigned char * client_public_key = NULL;
	if ((s->LURK_Request->ephemeral->method == secret_generated) && (SSL_is_server(s) == 1))
	{
        /*
		
		EVP_PKEY_get_raw_public_key(ckey, NULL, &len);
		buf = (unsigned char*)OPENSSL_malloc(len);
		if (buf == NULL)
		{
			return EXT_RETURN_FAIL;
		}
		if (EVP_PKEY_get_raw_public_key(ckey, buf, &len) != 1) {
			return EXT_RETURN_FAIL;
		}
		*/
		len = EVP_PKEY_get1_tls_encodedpoint(ckey, &client_public_key);
		if (len == 0) {
			return EXT_RETURN_FAIL;
		}
		if (LURK_ssl_generate_pkey(client_public_key, len, s->s3->group_id, s->LURK_Request, s->LURK_Response) == 0)
		{
			return EXT_RETURN_FAIL;
		}
		encodedPoint = (unsigned char *)OPENSSL_malloc((s->LURK_Response->ephemeral->server_share->key_len) * sizeof(unsigned char));
		if (encodedPoint == NULL)
		{
			return EXT_RETURN_FAIL;
		}
		memcpy(encodedPoint, s->LURK_Response->ephemeral->server_share->key_exchange, s->LURK_Response->ephemeral->server_share->key_len);
		encoded_pt_len = (size_t) s->LURK_Response->ephemeral->server_share->key_len;
