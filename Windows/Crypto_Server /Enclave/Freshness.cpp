...
EVP_MD_CTX *mdctx = EVP_MD_CTX_new();
	...
	EVP_DigestInit_ex(mdctx, md, NULL);
	EVP_DigestUpdate(mdctx, handshake_binary, handshake_size);

	size_t size = 0;
	unsigned char frist_part[10];
	frist_part[size] = (unsigned char)msg_type;
	size++;
	for (int i = 0; i < number_of_digits_for_size - 2; i++) {
		frist_part[i + size] = 0x0;
	}
	size = size + number_of_digits_for_size - 1;
	frist_part[size] = (unsigned char)size_of_new_thing_to_add;

	EVP_DigestUpdate(mdctx, frist_part, size);

	EVP_DigestUpdate(mdctx, new_thing_to_add, size_of_new_thing_to_add);
...
int calc_empty_hash(int md_number,unsigned char *out, size_t *outlen)
{
	unsigned char new_handshake_hash[EVP_MAX_MD_SIZE];
	unsigned int size_of_new_handshake_hash;
	if (hash_raw_handshake((unsigned char*)"", new_handshake_hash, &size_of_new_handshake_hash, md_number, 0) ==0){
		return 0;
	}
	*outlen =size_of_new_handshake_hash;
	memcpy(out,new_handshake_hash,size_of_new_handshake_hash);
	return 1;
