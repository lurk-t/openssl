...
Output: Secret *:
*/
int handle_secret_request2(uint16_t secret_request, Handshake *handshake, SharedSecret *ECDH_shared_secret, Secret *secret_list,
	unsigned char *handshake_hash, int &size_of_handshake_hash, enum Freshness fresh, unsigned char *old_rand,int md_index, int check_freshness,
	int operation_mode)
{
	...
	{
		//if secret_request == 0x09 && secret_request == 0x010 the empty hash would be generated inside the secret file
		if (secret_request != 0x09 && secret_request != 0x010) {
			if (calc_fresh_hash(handshake->handshake_binary, handshake_hash, fresh, old_rand, md_index, handshake->openssl_till_now) == 1)
			{
	...
	if (secret_request >= 3 && secret_request <= 12)
	{
		...
		if (test_handshake_secrets(secret_request, secret_list, ECDH_shared_secret->shared_secret, hashed_handshake,
			size_of_shared_secret,md_index, operation_mode, size_of_handshake_hash) == 1)
		{
			return 1;
		}
		else 
...
*/
int handle_secret_request(SInitCertVerifyRequest *request, SInitCertVerifyResponse *response, int check_freshness, unsigned char *handshake_hash_out) {
	return handle_secret_request2(request->secret_request, request->handshake, request->ephemeral->shared_secret, response->secret_list,
		handshake_hash_out, request->handshake_hash_size, request->freshness, request->old_rand, request->md_index, check_freshness, request->operation_mode);
}

//TODO 
//this should return the ID of the private key which signature is based on
int look_up_which_private_key(LURKTLS13Certificate *certificate) {
	return 1;
}
...
int handle_sign_request2(LURKTLS13Certificate *certificate, SignatureScheme sig_algo, Handshake *handshake,
	unsigned char *handshake_hash, int size_of_handshake_hash, Signature *Signature_respons, enum Freshness fresh,
	unsigned char *old_rand, int md_index, int check_freshness)
{
	//Test_Ocall();
	int key_ID = look_up_which_private_key(certificate);
	if (key_ID <= 0)// there was no corresponding key
	{
		return 0;
	}
	...
	if (check_freshness == 1) 
	{
		if (calc_fresh_hash(handshake->handshake_binary, handshake_hash, fresh, old_rand, md_index, handshake->till_now) == 1)
		{
			memcpy(hashed_handshake, handshake_hash, size_of_handshake_hash);
		}
		else
	...
	if (sgining_LURK(Signature_respons->signature, hashed_handshake, sig_algo, size_of_handshake_hash, &Signature_respons->signature_size) == 0 )
	{
		return 0;
	}
...
*/
int handle_sign_request(SInitCertVerifyRequest *request, SInitCertVerifyResponse *response, int check_freshness)
{
	return handle_sign_request2(request->certificate, request->sig_algo, request->handshake, request->handshake_hash, request->handshake_hash_size,
		response->signature, request->freshness, request->old_rand, request->md_index, check_freshness);
}
...
int add_hash_from_sig_verify_to_application_secrets(struct SInitCertVerifyRequest *req, struct SInitCertVerifyResponse *respons)
{
	...
	new_handshake_size =add_and_hash_sig_verify(req->handshake->handshake_binary, req->handshake->till_now,
		respons->signature->signature, respons->signature->signature_size, req->sig_algo,
		req->md_index, new_handshake_hash, &finish_size);
	...
	unusend_handshake_size = add_and_hash_finish(req->handshake->handshake_binary, new_handshake_size,
		respons->secret_list[6].secret_data, finish_size, req->md_index, new_handshake_hash, &new_handshake_size);
	...
	//req->handshake->size = unusend_handshake_size;
...
void init_certificate_verify(struct SInitCertVerifyRequest *req, int* a, struct SInitCertVerifyResponse *respons)
{
	...
	*/
	if (req->secret_request >= 3 && req->secret_request <= 10) {
