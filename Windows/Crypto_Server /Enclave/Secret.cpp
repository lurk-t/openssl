	...
	unsigned char *outsecret)
{
	...
	ret = EVP_PKEY_derive_init(pctx) <= 0
		...
		|| EVP_PKEY_CTX_set_hkdf_md(pctx, md) <= 0
		|| EVP_PKEY_CTX_set1_hkdf_key(pctx, insecret, (int)insecretlen) <= 0
		|| EVP_PKEY_CTX_set1_hkdf_salt(pctx, prevsecret, prevsecretlen) <= 0
		|| EVP_PKEY_derive(pctx, outsecret, &mdlen) <= 0;
	...
	int operation_mode, int &handshake_hash_size)
{
	...
	{
		memcpy(early_secret, (secret_list + secret_list_index_early_sec)->secret_data, secret_len);
		...
		operation_mode == OPRATION_MODE_1_EARLY_2_HS_3_SIG_AP_4_RESUMPTION))
	{
		...
		(secret_list + 9)->secret_type = LURK_early_secret;
		memcpy((secret_list + 9)->secret_data, early_secret, secret_len);
		(secret_list + 10)->secret_type = LURK_handshake_secret;
		memcpy((secret_list + 10)->secret_data, Handshake_secret, secret_len);
		return 1;
	}
	if (key_req == 4 )
	{
		...
		(secret_list + 1)->secret_type = L_server_handshake_traffic_secret;
		memcpy((secret_list + 1)->secret_data, S_hts_gensecret, secret_len);
		//TODO maybe it is better to copy this from openssl 
		if (tls13_derive_finishedkey(md, (secret_list + 1)->secret_data, fin, hashsize) == 0) {
			goto err;
		}
	...
	if (key_req == 6 || key_req == 5 || key_req == 7)
	{
		...
		(secret_list + 2)->secret_type = L_client_application_traffic_secret_0;
		memcpy((secret_list + 2)->secret_data, C_ats_gensecret, secret_len);
		...
		(secret_list + 3)->secret_type = L_server_application_traffic_secret_0;
		memcpy((secret_list + 3)->secret_data, S_ats_gensecret, secret_len);
		...
		operation_mode == OPRATION_MODE_1_EARLY_2_HS_3_SIG_AP_4_RESUMPTION))
	{
		...
		memcpy((secret_list + 5)->secret_data, resumtion_gensecret, secret_len);
	}
