...
int ssl3_finish_mac(SSL *s, const unsigned char *buf, size_t len)
{
    ...
    if (s->s3->handshake_dgst == NULL) {
		...
		//LURK
		set_data_before_hash_update(buf, len, s->LURK_Request);
...
int ssl3_digest_cached_records(SSL *s, int keep)
{
    ...
    if (s->s3->handshake_dgst == NULL) {
		...
		//LURK
		set_data_before_hash_update(hdata, hdatalen, s->LURK_Request);
    }
    if (keep == 0) {
        BIO_free(s->s3->handshake_buffer);
        s->s3->handshake_buffer = NULL;
                             ...
                             unsigned char *p)
{
	...
	//LURK
	set_data_before_hash_update(sender, len, s->LURK_Request);
