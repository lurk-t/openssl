                                    ...
                                    unsigned char *iv, EVP_CIPHER_CTX *ciph_ctx)
{
	...
	if (SSL_is_server(s) == 1) {
		...
		if (sec_req == 4)
		{
			...
			s->LURK_Request->md_index = 0x04; //TODO change
			modify_Handshake_Hash_SInitCertVerifyRequest(s->LURK_Request, s->handshake_traffic_hash, sizeof(s->handshake_traffic_hash));
