...
SSL *SSL_new(SSL_CTX *ctx)
{
	...
	/*
	* LRUK inits 
	* 1- init the req and responce for LURK CS
	*/
	
	s->LURK_Request = new_SInitCertVerifyRequest();
	s->LURK_Response = new_SInitCertVerifyResponse(s->LURK_Request);
	init_InitCertificateVerifyRequest(s->LURK_Request);
...
void SSL_free(SSL *s)
{
	...
	LURK_free_request(s->LURK_Request);
	LURK_free_response(s->LURK_Response);
